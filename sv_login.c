	...
	for (cl = svs.clients; cl - svs.clients < MAX_CLIENTS; cl++)
	{
		...
		for (i = 0, acc = accounts; i < num_accounts; i++, acc++)
			if ( (acc->use == use_log && !strncmp(acc->login, cl->login, CLIENT_LOGIN_LEN))
				/* FIXME IPv6 */
			        || (acc->use == use_ip && !strcmp(acc->login, va("%d.%d.%d.%d", cl->realip.address.ip[0], cl->realip.address.ip[1], cl->realip.address.ip[2], cl->realip.address.ip[3]))) )
				break;
...
qbool SV_Login(client_t *cl)
{
	...
	/* FIXME IPv6 */
	ip = va("%d.%d.%d.%d", cl->realip.address.ip[0], cl->realip.address.ip[1], cl->realip.address.ip[2], cl->realip.address.ip[3]);
